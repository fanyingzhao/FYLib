...
if (!signature) { [self doesNotRecognizeSelector:sel]; return _return_; } \
NSInvocation* invocation = [NSInvocation invocationWithMethodSignature:signature];\
if (!invocation) { [self doesNotRecognizeSelector:sel]; return _return_; } \
invocation.target = self;\
invocation.selector = sel;\
va_list arguments;\
va_start(arguments, _last_arg_);\
[NSObject setInv:invocation withSig:signature andArgs:arguments];\
va_end(arguments);

- (id)performSelectorWithArgs:(SEL)sel, ... {
    return [self performSelectorWithArgs:sel onThread:[NSThread currentThread] waitUntiDone:YES];
}

- (id)performSelectorWithArgsOnMainThread:(SEL)sel waitUntiDone:(BOOL)wait, ... {
    return [self performSelectorWithArgs:sel onThread:[NSThread mainThread] waitUntiDone:wait];
}
...
- (void)performSelectorWithArgs:(SEL)sel onThread:(NSThread*)thread afterDelay:(NSTimeInterval)delay waitUntiDone:(BOOL)wait modes:(NSArray<NSString *> *)array, ... {
    ...
    [invocation performSelector:@selector(invoke) onThread:thread withObject:nil waitUntilDone:wait modes:array];
}
